...
def CC(d1, d2, L, W, sigma2):
    """
    Calculate the correlation coefficient (CC) between two 2D arrays.

    Parameters:
    - d1, d2: 2D arrays of data values.
    - L: Number of rows in the arrays.
    - W: Number of columns in the arrays.
    - sigma2: Product of the standard deviations of d1 and d2.
...
def CMP_distance(r, x, y, data1, data2, mask):
    ...
    Returns:
    - s: Distance matrix for each radius.
...
def CMP_CC(r1, r, x, y, data1, data2, mask):
    """
    Calculate the CMP correlation coefficient (CC) for a given radius.
...
def Global_CC(data1, data2, L, W):
    """
    Calculate the global correlation coefficient (CC) for the entire dataset.
